        ...
        ReorderableLazyListState(
            ...
            shouldItemMove = when (orientation) {
                Orientation.Vertical -> { draggingItem, item ->
                    item.center.y >= draggingItem.top && item.center.y < draggingItem.bottom
                }

                Orientation.Horizontal -> { draggingItem, item ->
                    item.center.x >= draggingItem.left && item.center.x < draggingItem.right
                }
            },
        )
    }
...
fun LazyItemScope.ReorderableItem(
    ...
    val offsetModifier = if (dragging) {
        Modifier
            .zIndex(1f)
            .then(Modifier.graphicsLayer { translationY = state.draggingItemOffset.y })
    } else if (key == state.previousDraggingItemKey) {
        Modifier
            .zIndex(1f)
            .then(Modifier.graphicsLayer { translationY = state.previousDraggingItemOffset.value.y })
    } else {
        animateItemModifier
    }
